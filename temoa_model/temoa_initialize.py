...
def CreateCosts ( M ):
	...
	1. Collect all possible cost indices (CostFixed, CostVariable, CostEmission, SubisidesFixed, SubsidiesVariable)
	...
	CE = M.CostEmissions
	SF = M.SubsidiesFixed
	SV = M.SubsidiesVariable

	# Step 1
	fixed_indices = set( M.CostFixed_rptv )
	var_indices   = set( M.CostVariable_rptv )
	emis_indices = set(M.CostEmissions_rpe)
	fixed_indices_sub = set( M.SubsidiesFixed_rptv )
	var_indices_sub   = set( M.SubsidiesVariable_rptv )
	...
	unspecified_fixed_prices_sub = fixed_indices_sub.difference( SF.sparse_iterkeys() )
	unspecified_var_prices_sub   = var_indices_sub.difference( SV.sparse_iterkeys() )
	...
	if unspecified_fixed_prices_sub:
		# CF._constructed = False
		for r, p, t, v in unspecified_fixed_prices_sub:
			if (r, t, v) in M.SubsidiesFixedVintageDefault:
				SF[r, p, t, v] = M.SubsidiesFixedVintageDefault[r, t, v]
		# CF._constructed = True

	if unspecified_var_prices:
		# CV._constructed = False
		for r, p, t, v in unspecified_var_prices:
			if (r, t, v) in M.SubsidiesVariableVintageDefault:
				SV[r, p, t, v] = M.SubsidiesVariableVintageDefault[r, t, v]
		# CV._constructed = True
...
def SubsidiesFixedIndices ( M ):
	return M.activeActivity_rptv

def CostVariableIndices ( M ):
	return M.activeActivity_rptv

def SubsidiesVariableIndices ( M ):
	return M.activeActivity_rptv
