...
def temoa_create_model(name="Temoa"):
  ...
  """
    ...
    # Define technology subsidies parameters
    M.SubsidiesFixed_rptv = Set(dimen=4, initialize=SubsidiesFixedIndices)
    M.SubsidiesFixed = Param(M.SubsidiesFixed_rptv, mutable=True, default = 0.00)

    M.SubsidiesFixedVintageDefault_rtv = Set(
        dimen=3, initialize=lambda M: set((r, t, v) for r, p, t, v in M.SubsidiesFixed_rptv)
    )
    M.SubsidiesFixedVintageDefault = Param(M.SubsidiesFixedVintageDefault_rtv)

    M.SubsidiesInvest_rtv = Set(dimen=3, initialize=SubsidiesInvestIndices)
    M.SubsidiesInvest = Param(M.SubsidiesInvest_rtv, default = 0.00)

    M.SubsidiesVariable_rptv = Set(dimen=4, initialize=SubsidiesVariableIndices)
    M.SubsidiesVariable = Param(M.SubsidiesVariable_rptv, mutable=True, default = 0.00)

    M.SubsidiesVariableVintageDefault_rtv = Set(
        dimen=3, initialize=lambda M: set((r, t, v) for r, p, t, v in M.SubsidiesVariable_rptv)
    )
    M.SubsidiesVariableVintageDefault = Param(M.SubsidiesVariableVintageDefault_rtv)
    ...
    M.MinShareGroupTarget = Param(M.RegionalIndices,M.time_optimize,M.tech_groups ,M.groups)    
    M.MinShareGroupWeight = Param(M.RegionalIndices,M.tech_groups ,M.groups, default = 0)
    M.MaxShareGroupTarget = Param(M.RegionalIndices,M.time_optimize,M.tech_groups ,M.groups)    
    M.MaxShareGroupWeight = Param(M.RegionalIndices,M.tech_groups ,M.groups, default = 0)
    M.LinkedTechs = Param(M.RegionalIndices, M.tech_all, M.commodity_emissions)
    M.MinInvestShareGroupTarget = Param(M.RegionalIndices,M.vintage_all,M.tech_groups ,M.groups)    
    M.MinInvestShareGroupWeight = Param(M.RegionalIndices,M.tech_groups ,M.groups, default = 0)
    M.MaxInvestShareGroupTarget = Param(M.RegionalIndices,M.vintage_all,M.tech_groups ,M.groups)    
    M.MaxInvestShareGroupWeight = Param(M.RegionalIndices,M.tech_groups ,M.groups, default = 0)
    ...
    M.MaxActivityGroup_pg = Set(
        dimen=2, initialize=lambda M: M.MaxGenGroupTarget.sparse_iterkeys()
    )
    M.MaxActivityGroup = Constraint(
        M.MaxActivityGroup_pg, rule=MaxActivityGroup_Constraint
    )

    M.MinShareGroup_pg = Set(
        dimen=4, initialize=lambda M: M.MinShareGroupTarget.sparse_iterkeys()
    )
    M.MinShareGroup = Constraint(
        M.MinShareGroup_pg, rule=MinShareGroup_Constraint
    )

    M.MaxShareGroup_pg = Set(
        dimen=4, initialize=lambda M: M.MaxShareGroupTarget.sparse_iterkeys()
    )
    M.MaxShareGroup = Constraint(
        M.MaxShareGroup_pg, rule=MaxShareGroup_Constraint
    )

    M.MinInvestShareGroup_pg = Set(
        dimen=4, initialize=lambda M: M.MinInvestShareGroupTarget.sparse_iterkeys()
    )
    M.MinInvestShareGroup = Constraint(
        M.MinInvestShareGroup_pg, rule=MinInvestShareGroup_Constraint
    )

    M.MaxInvestShareGroup_pg = Set(
        dimen=4, initialize=lambda M: M.MaxInvestShareGroupTarget.sparse_iterkeys()
    )
    M.MaxInvestShareGroup = Constraint(
        M.MaxInvestShareGroup_pg, rule=MaxInvestShareGroup_Constraint
    )
