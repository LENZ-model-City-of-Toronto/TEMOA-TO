			...
			if self.optimizer:
				if self.options.neos:
					...
					if self.options.solver =='cbc':
						#self.optimizer.options['threads'] = 6 # Minimum number of thread 
						#self.optimizer.options['ratio'] = 1.e-3 # Minimum number of thread 
						sym_labels= False
						#sym_labels=self.options.keepPyomoLP

					elif self.options.solver == 'cplex':
						self.optimizer.options['lpmethod'] = 4 # 1 primal simplex 2 dual simplex 4 barrier
						self.optimizer.options['threads'] = 6 # Minimum number of thread 
						self.optimizer.options['preprocessing dual'] = -1
						self.optimizer.options['barrier startalg'] = 1 
						self.optimizer.options['barrier alg'] = 0
						self.optimizer.options['solutiontype'] = 2
						self.optimizer.options['barrier convergetol'] = 1.e-3
						self.optimizer.options['feasopt tolerance'] = 1.e-3

						sym_labels=self.options.keepPyomoLP
					else :
						sym_labels=self.options.keepPyomoLP

					#self.result= self.optimizer.tune( self.instance, keepfiles=self.options.keepPyomoLP)
