...
def PeriodCost_rule(M, p):
    ...
    loan_costs = sum(
        ...
        * (
            (value(M.CostInvest[r, S_t, S_v]) - value(M.SubsidiesInvest[r, S_t, S_v]))
    ...
    fixed_costs = sum(
        M.V_Capacity[r, S_t, S_v]
        * (
            (value(M.CostFixed[r, p, S_t, S_v]) - value(M.SubsidiesFixed[r, p, S_t, S_v]))
            * (
    ...
    variable_costs = sum(
        ...
        * (
            (value(M.CostVariable[r, p, S_t, S_v]) - value(M.SubsidiesVariable[r, p, S_t, S_v]))
            * (
                value(MPL[r, p, S_t, S_v])
                if not GDR
                else (x ** (P_0 - p + 1) * (1 - x ** (-value(MPL[r, p, S_t, S_v]))) / GDR)
    ...
    # Code from the ACES model
    # In Temoa, there are five types of processes that can produce emissions:
    ...
    if GRM == 0:
        if p == periods[0]:
            ...
            if (r, p_prev, t) in CapPT.keys():
                expr = CapPT[r, p, t] <= GRS*(p-p_prev) + CapPT[r, p_prev, t]
            else:
                expr = CapPT[r, p, t] <= GRS


    # Exponential growth constraint
    elif GRM != 0:
        if p == periods[0]:
            ...
            p_prev = periods[p_prev - 1]
            if (r, p_prev, t) in CapPT.keys():
                expr = CapPT[r, p, t] <= GRM**(p-p_prev)* CapPT[r, p_prev, t]
            else:
                expr = CapPT[r, p, t] <= GRS * GRM           
...
where :math:`g` represents the assigned technology group and :math:`MGT_r`
refers to the :code:`MaxGenGroupTarget` parameter.
"""
    ...
    max_act = value(M.MaxGenGroupTarget[p, g])
    expr = activity_p + activity_p_annual <= max_act
    return expr


def MinShareGroup_Constraint(M, r, p, t, g):
    r"""

The MinShareGroup constraint sets a minimum share limit for a user-defined
technology within a group of technology. 
"""

    activity_p = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o]* M.MinShareGroupWeight[r ,S_t , g]
        for S_t in M.tech_groups if (S_t not in M.tech_annual) and ((r, p, S_t) in M.processVintages.keys()) and (g in M.groups.keys()) 
        for S_v in M.processVintages[r, p, S_t]
        for S_i in M.processInputs[r, p, S_t, S_v]
        for S_o in M.ProcessOutputsByInput[r, p, S_t, S_v, S_i]
        for s in M.time_season
    ...
    activity_p_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o]* M.MinShareGroupWeight[r ,S_t , g]
        for S_t in M.tech_groups if (S_t in M.tech_annual) and ((r, p, S_t) in M.processVintages.keys()) and (g in M.groups.keys()) 
        for S_v in M.processVintages[r, p, S_t]
        for S_i in M.processInputs[r, p, S_t, S_v]
        for S_o in M.ProcessOutputsByInput[r, p, S_t, S_v, S_i]
    )
    ...
    activity_p_tech = 0
    activity_p_tech_annual = 0
    if (t not in M.tech_annual) and ((r, p, t) in M.processVintages.keys()):
        activity_p_tech = sum(
            M.V_FlowOut[r, p, s, d, S_i, t, S_v, S_o]
            for S_v in M.processVintages[r, p, t]
            for S_i in M.processInputs[r, p, t, S_v]
    ...
    if (t in M.tech_annual) and ((r ,p, t) in M.processVintages.keys()):
        activity_p_tech_annual = sum(
            M.V_FlowOutAnnual[r, p, S_i, t, S_v, S_o]
            for S_v in M.processVintages[r, p, t]
            for S_i in M.processInputs[r, p, t, S_v]
            for S_o in M.ProcessOutputsByInput[r, p, t, S_v, S_i]
    ...
    min_share = value(M.MinShareGroupTarget[r, p, t, g])
    expr =  (activity_p_tech + activity_p_tech_annual) - min_share * (activity_p + activity_p_annual) >= 0
    return expr
    

def MinInvestShareGroup_Constraint (M, r, v, t, g):
    r"""

This constraint sets an lower bound on a technology market share sales among a group of technology.

""" 
    p = v
    capacity_group_v = sum(
        M.V_Capacity[r, S_t, S_v]* M.MinInvestShareGroupWeight[r, S_t, g]
        for S_t in M.tech_groups if (r, p, S_t) in M.processVintages.keys() and (g in M.groups.keys()) 
        for S_v in M.processVintages[r, p, S_t]                                                                                     
    )

    min_share = value(M.MinInvestShareGroupTarget[r, v, t, g])

    if ((r, p, t, v) in M.processVintages.keys()) :
        expr = M.V_Capacity[r, t, v] >= min_share * capacity_group_v
...
def MaxInvestShareGroup_Constraint (M, r, v, t, g):
    r"""

This constraint sets an upper bound on a technology market share sales among a group of technology.

""" 
    p = v
    capacity_group_v = sum(
        M.V_Capacity[r, S_t, S_v]* M.MaxInvestShareGroupWeight[r, S_t, g]
        for S_t in M.tech_groups if (r, p, S_t) in M.processVintages.keys() and (g in M.groups.keys())
        for S_v in M.processVintages[r, p, S_t]                                                                                             
    )

    max_share = value(M.MaxInvestShareGroupTarget[r, v, t, g])

    if ((r, p, t, v) in M.processVintages.keys()) :
        expr = M.V_Capacity[r, t, v]  <= max_share * capacity_group_v
...
def MaxShareGroup_Constraint(M, r, p, t, g):
    r"""

The MaxShareGroup constraint sets a maximum share limit for a user-defined
technology within a group of technology. 
"""

    activity_p = sum(
        M.V_FlowOut[r, p, s, d, S_i, S_t, S_v, S_o]* M.MaxShareGroupWeight[r ,S_t , g]
        for S_t in M.tech_groups if (S_t not in M.tech_annual) and ((r, p, S_t) in M.processVintages.keys()) and (g in M.groups.keys()) 
        for S_v in M.processVintages[r, p, S_t]
        for S_i in M.processInputs[r, p, S_t, S_v]
        for S_o in M.ProcessOutputsByInput[r, p, S_t, S_v, S_i]
    ...
    activity_p_annual = sum(
        M.V_FlowOutAnnual[r, p, S_i, S_t, S_v, S_o]* M.MaxShareGroupWeight[r ,S_t , g]
        for S_t in M.tech_groups if (S_t in M.tech_annual) and ((r, p, S_t) in M.processVintages.keys()) and (g in M.groups.keys()) 
        for S_v in M.processVintages[r, p, S_t]
        for S_i in M.processInputs[r, p, S_t, S_v]
        for S_o in M.ProcessOutputsByInput[r, p, S_t, S_v, S_i]
    ...
    max_share = value(M.MaxShareGroupTarget[r, p, t, g])
    expr =  (activity_p_tech + activity_p_tech_annual) - max_share * (activity_p + activity_p_annual) <= 0
    return expr
