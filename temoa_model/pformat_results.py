...
def stringify_data ( data, ostream=SO, format='plain' ):
	...
	# format is currently unused, but will be utilized to implement things like csv
	...
	for i, (v, val) in enumerate( data ):
		# Conversion in decimal for data with higher value than 1.e-6	
...
def pformat_results ( pyomo_instance, pyomo_result, options ):
	...
	if hasattr(options, 'file_location') and os.path.join('temoa_model', 'config_sample_myopic') not in options.file_location: 
		...
		for r, t, v in m.CostInvest.sparse_iterkeys():   # Returns only non-zero values
			...
			svars[	'Costs'	][ 'V_UndiscountedInvestmentCostsByProcess', r, t, v] += icost
			...
			svars[	'Costs'	][ 'V_DiscountedInvestmentCostsByProcess', r, t, v] += icost
        ...
        # Calculate the costs associated with emissions. -- Code from the ACES model
		...
		for r, p, e, t, v in system_emissions.keys():
			if (r, p, e) in m.CostEmissions.sparse_iterkeys():
				...
				svars['Costs']['V_DiscountedEmissionsCostsByProcess', r, t, p] += ecost
		...
		# Calculate model subsidies:	
		for r, t, v in m.SubsidiesInvest.sparse_iterkeys():   # Returns only non-zero values

			isubsidies = value( m.V_Capacity[r, t, v] )
			if abs(isubsidies) < epsilon: continue
			isubsidies *= value( m.SubsidiesInvest[r, t, v] )*(
				(
					1 -  x**( -min( value(m.LifetimeProcess[r, t, v]), P_e - v ) )
				)/(
					1 -  x**( -value( m.LifetimeProcess[r, t, v] ) ) 
			...
			svars[	'Costs'	][ 'V_UndiscountedInvestmentSubsidiesByProcess', r, t, v] += isubsidies
	
			isubsidies *= value( m.LoanAnnualize[r, t, v] )
			isubsidies *= (
			  value( LLN[r, t, v] ) if not GDR else
			    (x **(P_0 - v + 1) * (1 - x **(-value( LLN[r, t, v] ))) / GDR)
			)
	
			svars[	'Costs'	][ 'V_DiscountedInvestmentSubsidiesByProcess', r, t, v] += isubsidies


		for r, p, t, v in m.SubsidiesFixed.sparse_iterkeys():
			fsubsidies = value( m.V_Capacity[r, t, v] )
			if abs(fsubsidies) < epsilon: continue
	
			fsubsidies *= value( m.SubsidiesFixed[r, p, t, v] )
			svars[	'Costs'	][ 'V_UndiscountedFixedSubsidiesByProcess', r, t, v] += fsubsidies * value( MPL[r, p, t, v] )
			...
			svars[	'Costs'	][ 'V_DiscountedFixedSubsidiesByProcess', r, t, v] += fsubsidies

		for r, p, t, v in m.SubsidiesVariable.sparse_iterkeys():
			if t not in m.tech_annual:
				vsubsidies = sum(
					value (m.V_FlowOut[r, p, S_s, S_d, S_i, t, v, S_o])
					for S_i in m.processInputs[r, p, t, v]
			...
			if abs(vsubsidies) < epsilon: continue
	
			vsubsidies *= value( m.SubsidiesVariable[r, p, t, v] )
			svars[	'Costs'	][ 'V_UndiscountedVariableSubsidiesByProcess', r, t, v] += vsubsidies * value( MPL[r, p, t, v] )
			vsubsidies *= (
			  value( MPL[r, p, t, v] ) if not GDR else
			    (x **(P_0 - p + 1) * (1 - x **(-value( MPL[r, p, t, v] ))) / GDR)
			  ) 
			svars[	'Costs'	][ 'V_DiscountedVariableSubsidiesByProcess', r, t, v] += vsubsidies

	if options.saveDUALS:
		duals = collect_result_data( Cons, con_info, epsilon=1e-5 )
